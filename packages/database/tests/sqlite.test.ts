...
describe('sqlite connector', () => {
  ...
  describe("users table", () => {
    ...
    it('should NOT throw an error when users table exist in database', async () => {
      const db = createDatabase(sqlite({}));
      await db.sql`CREATE TABLE IF NOT EXISTS users ("id" TEXT PRIMARY KEY)`;
      expect(checkDbAndTables(db, "sqlite")).toBeTruthy()
    });

    it.only('should throw an error when users table does not have an id field', async () => {
      const db = createDatabase(sqlite({}));
    ...
    it.only('should throw an error when users table does not have an id field with of text', async () => {
      const db = createDatabase(sqlite({}));
    ...
    it.only('should throw an error when users table does not have an email field', async () => {
      const db = createDatabase(sqlite({}));
      await db.sql`CREATE TABLE IF NOT EXISTS users ("id" TEXT PRIMARY KEY)`;
    ...
    it.only('should throw an error when users table does not have an email field with of text', async () => {
      const db = createDatabase(sqlite({}));
