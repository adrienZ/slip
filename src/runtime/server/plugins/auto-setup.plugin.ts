...
export default defineNitroPlugin(async (nitro: NitroApp) => {
  ...
  if (config.slipAuth.database.dialect === "sqlite" || config.slipAuth.database.dialect === "libsql" || config.slipAuth.database.dialect === "bun-sqlite") {
    await db.sql`CREATE TABLE IF NOT EXISTS slip_auth_users ("id" TEXT NOT NULL PRIMARY KEY, "email" TEXT NOT NULL UNIQUE, "email_verified" BOOLEAN DEFAULT 0, "password" TEXT, "created_at" TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, "updated_at" TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)`;
